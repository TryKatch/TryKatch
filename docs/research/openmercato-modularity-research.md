...
| Assistant tools | Modules now declare stable tool IDs, operation bindings, risk, and confirmation policy. OpenAPI emits module/tool metadata and generates strict provider-neutral function schemas. Three read-only tools prove the contract. | Add permission-filtered runtime discovery, provider adapters, execution budgets, audit/trace attributes, and a pending-action approval UI before enabling mutations. Tenant configuration may only narrow code-declared policy. OpenMercato implements the useful precedent in its agent definition and policy gate. [Agent contract](https://github.com/open-mercato/open-mercato/blob/main/packages/ai-assistant/src/modules/ai_assistant/lib/ai-agent-definition.ts), [policy gate](https://github.com/open-mercato/open-mercato/blob/main/packages/ai-assistant/src/modules/ai_assistant/lib/agent-policy.ts) |
...
5. **P2 — complete assistant execution after the read-only contract:** keep the implemented strict read-only allowlist, then add provider adapters and staged, explicitly approved mutations with complete audit and trace coverage.
