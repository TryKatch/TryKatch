...
The coordinated release target is **Trykatch 0.1.0-preview.33**, making the generated project name the default login, navigation and email brand while preserving an explicit `--display-name` override. It also retains the preview.32 generated-application routing boundary and the preview.31 organization settings, shared floating controls, developer onboarding, provider-neutral AI Help and Storybook. Publication requires successful exact-commit main CI and sealed tag qualification; target metadata is not proof of publication. See the [release notes](releases/0.1.0-preview.33.md).
...
- Coordinated preview target: `0.1.0-preview.33`; the preceding published package is `0.1.0-preview.32` until successful publication.
