...
function specification(method = 'get', declaration = { name: 'trykatch_list_things', description: 'List things.', risk: 'read-only', requiresHumanConfirmation: false }) {
  return {
    openapi: '3.1.1',
    info: { title: 'Test API', version: 'v1' },
    'x-trykatch-modules': [{ id: 'things', version: '1.0.0' }],
    paths: {
      '/api/v1/things': {
        [method]: {
          operationId: 'Things_List',
          'x-trykatch-module': 'things',
          'x-trykatch-assistant-tool': declaration,
...
test('exports strict tools and request bindings from explicit OpenAPI metadata', () => {
  ...
  assert.equal(contract.tools[0].name, 'trykatch_list_things')
  ...
  assert.equal(contract.bindings.trykatch_list_things.moduleId, 'things')
  assert.equal(contract.bindings.trykatch_list_things.authorization, 'api-enforced')
})
