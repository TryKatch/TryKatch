    ...
    public async Task RuntimeRoleCannotCrossOrganizationBoundary()
    {
        ...
        await using (NpgsqlConnection owner = new(postgres.GetConnectionString()))
        {
            ...
            setup.CommandText = """
                ...
                CREATE ROLE trykatch_test_runtime LOGIN PASSWORD 'runtime-test' NOBYPASSRLS;
                GRANT USAGE ON SCHEMA app TO trykatch_test_runtime;
                GRANT SELECT, INSERT, UPDATE, DELETE ON app.projects TO trykatch_test_runtime;
                """;
            await setup.ExecuteNonQueryAsync();
        }
        ...
        NpgsqlConnectionStringBuilder connectionBuilder = new(postgres.GetConnectionString())
        {
            Username = "trykatch_test_runtime",
