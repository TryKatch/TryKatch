...
using TrykatchApp.Infrastructure.Persistence.Migrations.Platform;
...
public sealed class PostgresIsolationInspectionTests
{
    ...
    public async Task PlatformResourceDoesNotPermitOrganizationRuntimeAccess()
    {
        ...
        TrykatchModuleDescriptor platform = new ProjectsModule().Descriptor with
        {
            Id = "reporting", DefaultDataOwnership = TrykatchDataOwnership.Platform,
            DataResources = [new("reports", "platform", "reports", TrykatchDataOwnership.Platform, AccessRule: TrykatchDataAccessRule.PlatformOnly)]
        };
        TrykatchModuleDescriptor[] modules = [new ProjectsModule().Descriptor, platform];
