...
internal static class SignedModuleTestArtifacts
{
    ...
    public static void Create(string root, string moduleId, string version, string? invalidEvidence = null)
    {
        ...
        using (MemoryStream metadata = new(JsonSerializer.SerializeToUtf8Bytes(new { name = "@trykatchapp/module-reporting", version = invalidEvidence == "frontend-version" ? "9.9.9" : version })))
        ...
        [
            ...
            ($"pkg:npm/%40trykatchapp/module-reporting@{version}", "@trykatchapp/module-reporting", frontend)
        ];
        ...
        JsonNode catalog = JsonNode.Parse(File.ReadAllText(Path.Combine(root, "trykatch.modules.json")))!;
        ...
        File.WriteAllText(Path.Combine(root, "trykatch.modules.json"), catalog.ToJsonString());
