...
namespace TrykatchApp.Migrator;

internal static class ModuleMigrationExecutor
{
    private const string LockName = "trykatch:module-migrations:v1";

    public static async Task ApplyAsync(
        string connectionString,
        IReadOnlyList<ITrykatchModule> enabledModules,
        CancellationToken cancellationToken = default)
    {
        ...
        List<AppliedTrykatchModuleMigration> applied = [];
        ...
        IReadOnlyList<PendingTrykatchModuleMigration> pending =
            TrykatchModuleMigrationPlan.Build(enabledModules, applied);
        foreach (PendingTrykatchModuleMigration migration in pending)
        {
