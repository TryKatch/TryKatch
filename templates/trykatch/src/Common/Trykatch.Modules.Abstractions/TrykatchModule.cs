...
public sealed record ExtensionPointDescriptor(
    string Id,
    string Description,
    ExtensionPointKind Kind,
    ModuleCapabilities Surface);
...
public enum ModuleDataOwnership
{
    Organization,
    Platform,
    Global,
...
public sealed record DataResourceDescriptor(
    ...
    string Schema,
    string Table,
    ModuleDataOwnership Ownership,
    string? EntityType = null,
    string? IsolationPolicy = null,
    ModuleDataAccessRule? AccessRule = null);

public enum ModuleDataAccessRule
{
    PlatformOnly,
    IdentityOnly,
    GlobalReadOnly,
...
public sealed record InstalledDataResource(string ModuleId, DataResourceDescriptor Resource);
...
public sealed record ModuleDescriptor(
    ...
    IReadOnlyList<ExtensionPointDescriptor> ExtensionPoints)
{
    public IReadOnlyList<AssistantToolDescriptor> AssistantTools { get; init; } = [];
    public ModuleDataOwnership? DefaultDataOwnership { get; init; }
    public IReadOnlyList<DataResourceDescriptor> DataResources { get; init; } = [];
    public string Publisher { get; init; } = "try" + "katch";
...
public sealed class ModuleAttribute(string moduleId) : Attribute
{
...
public sealed class ModuleCatalog
{
    ...
    public ModuleCatalog(IEnumerable<IModule> modules)
    {
        ...
        IModule[] supplied = modules.ToArray();
        ValidateDescriptors(supplied);

        Dictionary<string, IModule> byId = supplied.ToDictionary(
    ...
    private static void ValidateDescriptors(IReadOnlyCollection<IModule> modules)
    {
        ...
        foreach (IModule module in modules)
        {
            ...
            foreach (PermissionDescriptor permission in descriptor.Permissions)
            {
            ...
            foreach (ExtensionPointDescriptor point in descriptor.ExtensionPoints)
            {
                ...
                if (point.Surface == ModuleCapabilities.None
            ...
            if (descriptor.AssistantTools.Count > 0
                && !descriptor.Capabilities.HasFlag(ModuleCapabilities.Assistant))
            ...
            foreach (AssistantToolDescriptor tool in descriptor.AssistantTools)
            {
                ...
                if (tool.Risk != AssistantToolRisk.ReadOnly && !tool.RequiresHumanConfirmation)
    ...
    private static void ValidateDataOwnership(ModuleDescriptor descriptor)
    {
        bool hasData = descriptor.Capabilities.HasFlag(ModuleCapabilities.Data);
    ...
    private static void ValidateDependencies(IReadOnlyDictionary<string, IModule> modules)
    {
    ...
    private static List<IModule> OrderByDependencies(IReadOnlyDictionary<string, IModule> modules)
    {
        ...
        void Visit(string moduleId)
        {
            ...
            IModule module = modules[moduleId];
...
public static class ModuleServiceCollectionExtensions
{
        ...
        IEnumerable<IModule> modules)
    {
        ...
        ModuleCatalog catalog = new(modules);
