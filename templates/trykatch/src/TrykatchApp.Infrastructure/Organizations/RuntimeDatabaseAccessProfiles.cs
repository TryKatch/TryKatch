...
public static class RuntimeDatabaseAccessProfiles
{
    ...
    {
        TrykatchDataResourceDescriptor? resource = resources.GetValueOrDefault(relation);
        return kind switch
        {
    ...
    private static IReadOnlySet<string> OrganizationPermissions(
        string relation, TrykatchDataResourceDescriptor? resource) => relation switch
    {
        ...
        _ when resource?.Ownership == TrykatchDataOwnership.Organization => Crud,
        _ when resource?.AccessRule == TrykatchDataAccessRule.GlobalReadOnly => Select,
        _ => None
    };
        ...
        string relation, TrykatchDataResourceDescriptor? resource) => relation switch
    {
        ...
        _ when resource?.AccessRule == TrykatchDataAccessRule.PlatformOnly => Crud,
        _ => None
    };
