...
internal sealed class OrganizationAccessResolver(PlatformDbContext dbContext, IPermissionCatalog permissionCatalog) : IOrganizationAccessResolver
{
    public async Task<OrganizationAccess?> ResolveAsync(Guid actorId, Guid organizationId, CancellationToken cancellationToken = default)
    {
        ...
        string[] permissions = await dbContext.Roles
            .AsNoTracking()
            .Where(x => roleIds.Contains(x.Id) && x.ArchivedAt == null && x.DeletedAt == null)
