...
namespace TrykatchApp.Api.OpenApi;

/// <summary>Adds stable product and installed-module metadata to the API contract.</summary>
public sealed class TrykatchOpenApiDocumentTransformer(TrykatchModuleCatalog catalog) : IOpenApiDocumentTransformer
{
    ...
    {
        document.Info.Title = "Trykatch API";
        document.Info.Version = "v1";
        document.Info.Description =
            "The versioned HTTP contract for Trykatch's platform and organization workspaces. " +
        ...
        object[] modules = catalog.Descriptors.Select(descriptor => new
        {
            ...
            capabilities = Enum.GetValues<TrykatchModuleCapabilities>()
                .Where(capability => capability != TrykatchModuleCapabilities.None && descriptor.Capabilities.HasFlag(capability))
...
/// </summary>
public sealed partial class TrykatchOpenApiOperationTransformer(TrykatchModuleCatalog catalog) : IOpenApiOperationTransformer
{
    public Task TransformAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        string? moduleId = context.Description.ActionDescriptor.EndpointMetadata
            .OfType<TrykatchModuleEndpointMetadata>()
            .Select(metadata => metadata.ModuleId)
            .FirstOrDefault()
            ?? context.Description.ActionDescriptor.EndpointMetadata
                .OfType<TrykatchModuleAttribute>()
        ...
        operation.AddExtension("x-trykatch-module", new JsonNodeExtension(JsonSerializer.SerializeToNode(moduleId)!));
        TrykatchAssistantToolDescriptor? tool = catalog.Descriptors
        ...
        operation.AddExtension(
            ...
            new JsonNodeExtension(JsonSerializer.SerializeToNode(new
            {
                ...
                {
                    TrykatchAssistantToolRisk.ReadOnly => "read-only",
                    TrykatchAssistantToolRisk.Mutating => "mutating",
                    TrykatchAssistantToolRisk.Destructive => "destructive",
