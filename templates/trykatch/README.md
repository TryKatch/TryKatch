...
Choose the customer-facing name independently of namespaces with `--display-name "Kamenta"`; see [product branding](docs/branding.md) ([Français](docs/branding.fr.md)).
...
- Workspace invitations let an authorized administrator select the role before sending and return a complete one-time URL rather than a standalone token. The optional SMTP adapter sends that URL and the selected role's friendly name in plain-text and HTML email; otherwise an administrator can copy the link. Acceptance assigns the saved role. New invitees provide first and last name plus a password, while existing identities sign in before accepting.
