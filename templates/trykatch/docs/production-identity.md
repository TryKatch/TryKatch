...
The plaintext maintenance operation intentionally does not re-encrypt existing certificate-wrapped records or delete old keys. Retain A for as long as any stored key or recoverable backup needs it, not merely until cookies expire: federation secrets, invitation material, organization-owned AI subscription keys, and backups may outlive browser sessions. Retirement of A requires a separately reviewed full re-encryption/retention procedure; no finite automatic retirement window is promised. OpenIddict rotation is a separate token-signing/encryption lifecycle and is not expanded by this DP rotation list.
...
Invitations and password resets share application-branded, table-based HTML with inline styles, a primary action and a copyable fallback URL. Both retain plain-text alternatives; invitations include the selected role's friendly name. Default branding follows the generated application's name and the UI's blue (`#315fba`), neutral surfaces and four-pixel corners. Operator configuration can override `Email:Branding:ApplicationName` (maximum 120 characters), `AccentColor` (six-digit hex), and optional `LogoUrl` (absolute HTTPS, no embedded credentials). Values are HTML-encoded and invalid branding fails startup. Use `Email__Branding__ApplicationName`, `Email__Branding__AccentColor`, and `Email__Branding__LogoUrl` as environment variables. Branding is not inferred from a recipient's personal appearance preferences; customize these alongside your product's frontend branding. External logos are optional so the default email does not depend on remotely loaded images.

Mailpit is for local capture, not production delivery. Any compatible SMTP provider can be configured without replacing the HTML renderer. For a free-tier deployment, SMTP2GO supports `Email:Host=mail.smtp2go.com`, `Port=587`, `Security=StartTls`, a verified sender in `From`, and an SMTP user's credentials as `Username`/`Password`. Store the secret server-side (prefer a mounted `PasswordFile`), never in frontend configuration. Disable provider link tracking for invitation/reset URLs so one-time credentials are not rewritten into tracking links. Verify domain authentication, account limits and bounce handling with the deployed provider before shipping. SMTP2GO is an example, not a mandatory template dependency or a provider we provision automatically. Its free plan currently has monthly and daily caps; check [SMTP settings](https://support.smtp2go.com/hc/en-gb/articles/223087627-SMTP-Settings) and [current free-plan limits](https://support.smtp2go.com/hc/en-gb/articles/223087947-Free-Plan) before deploying.
