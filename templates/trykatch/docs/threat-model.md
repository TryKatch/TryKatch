...
| Platform privilege escalation | Platform roles and permissions are separate from organization RBAC and database credentials; production rejects reused runtime roles; platform RLS recognizes only the fixed platform role, not a transaction/session bypass flag; a code-defined catalog rejects unknown grants; built-in roles are immutable; changing access updates the security stamp; the final active administrator is protected. |
...
| Database-owner compromise through the API | The API receives four scoped runtime credentials, never the migration owner; bootstrap, migrator, organization, platform, identity, and outbox secrets are distinct; runtime roles are NOINHERIT NOBYPASSRLS and own no relations; migration-time inspection and Testcontainers tests verify these invariants. |
...
| Supply-chain substitution | Explicit publisher signer/key/builder enrollment; authenticated NuGet and signed provenance; artifact identity and SPDX subject binding; freshness and signed vulnerability-result checks; verified byte snapshots restored through an exact local NuGet source/fresh cache and pinned npm archive; NuGet and pnpm lockfiles, independent release vulnerability audits, secret scanning, and tag-plus-digest container references. |
...
| Forged forwarding headers | The React profile binds to loopback but treats the host-NAT bridge gateway as untrusted. Its dedicated ingress-container IP is distinct from that gateway and reserved outside Docker's automatic allocation range, so unrelated services cannot acquire ingress trust. Nginx accepts forwarded client information only from that exact peer, emits one normalized client address, and otherwise derives the scheme and address from its own connection. The API accepts one symmetric forwarding hop only from the exact configured web-container IP. Direct API publishing is a diagnostic profile and requires its own trusted-proxy configuration. |
