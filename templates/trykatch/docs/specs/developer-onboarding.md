...
The first PR CI attempt exposed a qualification-harness portability defect: both first-feature jobs passed doctor/setup, then exited 127 because hosted runners did not provide `rg`. Reproduced the original committed assertions against a minimal `/usr/bin:/bin` PATH and actual fixture evidence; replaced the six assertion calls with standard literal grep checks, without skipping or weakening them. Added two regression cases that execute the actual harness assertion lines under that PATH: complete evidence passes and missing architecture evidence fails. Both cases pass and are required in CI's changes job. Direct command-not-found diagnostics established the cause; broad speculative instrumentation was unnecessary. Publication remains gated on the corrected remote run.
