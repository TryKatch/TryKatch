...
The React Compose profile binds port 8080 to loopback by default. Put a same-host TLS ingress in front of it, have that ingress remove any client-supplied forwarding headers, set `X-Forwarded-Proto` from the authenticated connection, and set one `X-Forwarded-For` value to the client address. The web proxy accepts only the value `https`, otherwise uses its own request scheme, and falls back to its peer address when `X-Forwarded-For` is absent. The API trusts only the exact web-container IP in `TRYKATCH_WEB_PROXY_IP`, requires symmetric `X-Forwarded-For` and `X-Forwarded-Proto` headers, and processes one forwarding hop. Keep that IP inside `TRYKATCH_NETWORK_SUBNET`; if the default subnet overlaps another Docker network, change both settings together. Do not publish the API container or expose the web container directly to an untrusted network. A multi-proxy or multi-host deployment must explicitly register every immediate trusted proxy and requalify its header and hop-limit contract instead of enabling trust-all forwarding.
