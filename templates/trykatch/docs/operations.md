...
Copy `.env.example` to `.env`, replace every placeholder, and set `TRYKATCH_RELEASE_VERSION` to an immutable release tag or commit identifier. Place separate PKCS#12 signing, encryption, and Data Protection certificates and their password files under `${TRYKATCH_SECRETS_PATH}`. Existing plaintext key rings require explicit privileged maintenance before startup. Follow [production identity](production-identity.md) for configuration, dry-run/apply, rotation, backup/restore, and SMTP TLS. Keep `.env`, certificates, database credentials, SMTP credentials, object-storage keys, and telemetry tokens out of Git and container images.

The React Compose profile binds port 8080 to loopback by default. That published port is for local diagnostics and is not a trusted forwarding path. Attach the same-host TLS ingress directly to the Compose network with the exact static `TRYKATCH_INGRESS_PROXY_IP`; its default `.2` address intentionally differs from `TRYKATCH_NETWORK_GATEWAY` at `.1`. `TRYKATCH_NETWORK_DYNAMIC_RANGE` confines automatic container allocation to `.128`–`.254`, keeping the reserved gateway, ingress, and web-proxy addresses outside that pool. Have the ingress remove client-supplied forwarding headers, set `X-Forwarded-Proto` from the authenticated connection, and set one `X-Forwarded-For` value to the client address. The web proxy accepts forwarded client information only from that dedicated ingress address. It resolves the address with Nginx's real-IP module, emits a single normalized `X-Forwarded-For` value, accepts only the exact scheme value `https`, and otherwise derives both values from its own connection. Host-NAT and loopback-published requests arrive from the distinct bridge gateway and remain untrusted. The API then trusts only the exact web-container IP in `TRYKATCH_WEB_PROXY_IP`, requires a symmetric header pair, and processes one hop. Keep the gateway, ingress proxy, web proxy, and dynamic range inside `TRYKATCH_NETWORK_SUBNET`; if the subnet overlaps another Docker network, change all five settings together. Do not publish the API container or expose the web container directly to an untrusted network. A multi-proxy or multi-host deployment must explicitly register every immediate trusted proxy and requalify its header and hop-limit contract instead of enabling trust-all forwarding.
