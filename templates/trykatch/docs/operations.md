...
The React Compose profile binds port 8080 to loopback by default. Put a same-host TLS ingress in front of it, have that ingress remove any client-supplied forwarding headers, set `X-Forwarded-Proto` from the authenticated connection, and set one `X-Forwarded-For` value to the client address. The web proxy accepts forwarded client information only when the connection peer is the exact `TRYKATCH_INGRESS_PROXY_IP`; the default is the dedicated Compose gateway. It resolves that address with Nginx's real-IP module, emits a single normalized `X-Forwarded-For` value, accepts only the exact scheme value `https`, and otherwise derives both values from its own connection. The API then trusts only the exact web-container IP in `TRYKATCH_WEB_PROXY_IP`, requires a symmetric header pair, and processes one hop. Keep the ingress proxy and web proxy IPs inside `TRYKATCH_NETWORK_SUBNET`; if the subnet overlaps another Docker network, change all three settings together. Do not publish the API container or expose the web container directly to an untrusted network. A multi-proxy or multi-host deployment must explicitly register every immediate trusted proxy and requalify its header and hop-limit contract instead of enabling trust-all forwarding.
