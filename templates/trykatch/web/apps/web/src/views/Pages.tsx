...
export function UserManagementPage() {
  ...
  return <>
    ...
    <Dialog open={inviteOpen} onOpenChange={setInviteOpen} title={invitationResult ? 'Invitation ready' : 'Invite a member'} description="The invitation is valid for seven days and can be accepted once.">{invitationResult ? <div className="token-result"><div className="delete-record-summary"><span>Recipient</span><strong>{invitationResult.invitation.email}</strong></div><p>{invitationResult.emailDelivered ? 'The invitation was emailed successfully. You can also copy the link below.' : 'Email delivery is not configured. Copy this link and share it with the invited person through a secure channel.'}</p><code>{invitationResult.invitationUrl}</code><Button variant="secondary" onClick={copyInvitationLink}><Copy size={14} /> {invitationCopied ? 'Link copied' : 'Copy invitation link'}</Button><div className="dialog-actions"><Button variant="primary" onClick={() => setInviteOpen(false)}>Done</Button></div></div> : <form className="dialog-form" onSubmit={submitInvite}><label>Email address<input name="email" type="email" autoComplete="email" required autoFocus /></label><small>The person will use the invitation link to create an account or sign in.</small>{invite.error && <div className="form-error" role="alert">{invite.error.message}</div>}<div className="dialog-actions"><Button type="button" variant="ghost" onClick={() => setInviteOpen(false)}>Cancel</Button><Button type="submit" variant="primary" disabled={invite.isPending}>{invite.isPending ? 'Creating…' : 'Create invitation'}</Button></div></form>}</Dialog>
