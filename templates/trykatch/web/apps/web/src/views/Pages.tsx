...
interface Invitation { id: string; roleId: string; email: string; createdAt: string; expiresAt: string; status: string; lifecycle: RecordLifecycle }
...
function detailsForManagedRecord(selection: { kind: 'member' | 'invitation' | 'role'; record: Member | Invitation | Role }, modules: readonly PermissionModuleDto[], roles: readonly Role[], translate: (key: string) => string) {
  ...
  if (selection.kind === 'invitation') {
    ...
    return [
      ...
      { label: 'Workspace role', value: translate(roles.find((role) => role.id === invitation.roleId)?.name ?? 'Role unavailable') },
  ...
  return [
    ...
    { label: 'Permissions', value: role.permissions.map((key) => translate(modules.flatMap((module) => module.permissions)
      .find((permission) => permission.key === key)?.name ?? 'Retired or unavailable permission')).join(', ') || translate('No permissions') },
...
export function UserManagementPage() {
  ...
  const [requestedTab, setTab] = useState<'members' | 'invitations' | 'roles'>('members')
  ...
  const canOpenManagement = canManageMembers || canManageRoles
  const canReadMembers = canOpenManagement && (access.data?.permissions.includes('members.read') ?? false)
  const canReadRoles = canOpenManagement && (access.data?.permissions.includes('roles.read') ?? false)
  const tab = requestedTab === 'roles' ? canReadRoles ? 'roles' : 'members' : canReadMembers ? requestedTab : 'roles'
  const members = useQuery({ enabled: canReadMembers, queryKey: ['members', 'active'], queryFn: () => customFetch<Member[]>('/api/v1/members?lifecycle=active', { method: 'GET' }) })
  const invitations = useQuery({ enabled: canReadMembers, queryKey: ['invitations', 'active'], queryFn: () => customFetch<Invitation[]>('/api/v1/invitations?lifecycle=active', { method: 'GET' }) })
  const roles = useQuery({ enabled: canReadRoles, queryKey: ['access-levels', 'active'], queryFn: () => customFetch<Role[]>('/api/v1/roles?lifecycle=active', { method: 'GET' }) })
  const activeRoles = roles
  const permissionCatalog = useQuery({ enabled: canReadRoles, queryKey: ['permission-catalog'], queryFn: () => customFetch<PermissionModuleDto[]>('/api/v1/permissions', { method: 'GET' }) })
  const invite = useMutation({
    mutationFn: ({ email, roleId }: { email: string; roleId?: string }) => customFetch<InvitationCreation>('/api/v1/invitations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, roleId, expiresInDays: 7 }) }),
  ...
  function submitInvite(event: FormEvent<HTMLFormElement>) {
    ...
    if (!canManageMembers || invite.isPending) return
    const data = new FormData(event.currentTarget)
    const roleId = canReadRoles ? data.get('roleId') : undefined
    if (canReadRoles && (typeof roleId !== 'string' || !roleId)) return
    invite.mutate({ email: String(data.get('email')), roleId: typeof roleId === 'string' ? roleId : undefined })
  }
  ...
  const invitationColumns: DataTableColumn<Invitation>[] = [
    ...
    { id: 'role', header: t('Workspace role'), cell: (invitation) => t(roles.data?.find((role) => role.id === invitation.roleId)?.name ?? 'Role unavailable'), sortValue: (invitation) => roles.data?.find((role) => role.id === invitation.roleId)?.name ?? '' },
  ...
  const roleColumns: DataTableColumn<Role>[] = [
    { id: 'name', header: t('Role'), hideable: false, cell: (role) => <div className="role-name-cell"><strong>{role.name}</strong><small>{role.description || t(role.isSystem ? 'System-managed role' : 'Custom workspace access')}</small></div>, sortValue: (role) => role.name, searchValue: (role) => `${role.name} ${role.description} ${role.permissions.join(' ')} ${rolePermissionMetadata(role, permissionCatalog.data ?? []).searchText}` },
    { id: 'type', header: t('Type'), cell: (role) => <div className="role-access-boundary">{role.lifecycle.status === 'Active' ? <Badge tone={role.isSystem ? 'neutral' : 'info'}>{t(role.isSystem ? 'System' : 'Custom')}</Badge> : <LifecycleBadge lifecycle={role.lifecycle} />}{role.isSystem ? <small>{t('Built-in · read-only')}</small> : !role.canAssign ? <small>{t('Outside your grant boundary')}</small> : null}</div>, sortValue: (role) => `${role.lifecycle.status}-${role.isSystem ? 'System' : 'Custom'}` },
    { id: 'permissions', header: t('Permissions'), cell: (role) => { const metadata = rolePermissionMetadata(role, permissionCatalog.data ?? []); return <div className="role-grant-summary"><strong>{role.permissions.length}</strong><small>{t(role.permissions.length === 1 ? 'permission' : 'permissions')} · {metadata.moduleCount} {t(metadata.moduleCount === 1 ? 'module' : 'modules')}</small></div> }, sortValue: (role) => role.permissions.length },
  ...
  if (!canOpenManagement) return <EmptyState title={t('Access unavailable')} description={t('Only workspace administrators can manage people and roles.')} />
  return <>
    ...
    <div className="user-management-tabs" role="tablist" aria-label={t('User management')}>
      {canReadMembers && <button role="tab" aria-selected={tab === 'members'} className={tab === 'members' ? 'active' : ''} onClick={() => changeTab('members')}><Users size={16} /> {t('People')} <Badge>{members.data?.length ?? 0}</Badge></button>}
      {canReadMembers && <button role="tab" aria-selected={tab === 'invitations'} className={tab === 'invitations' ? 'active' : ''} onClick={() => changeTab('invitations')}><Mail size={16} /> {t('Invitations')} <Badge>{invitations.data?.length ?? 0}</Badge></button>}
      {canReadRoles && <button role="tab" aria-selected={tab === 'roles'} className={tab === 'roles' ? 'active' : ''} onClick={() => changeTab('roles')}><ShieldCheck size={16} /> {t('Roles')} <Badge>{roles.data?.length ?? 0}</Badge></button>}
    </div>
    ...
    <Dialog open={inviteOpen} onOpenChange={setInviteOpen} title={t(invitationResult ? 'Invitation ready' : 'Invite a member')} description={t('The invitation is valid for seven days and can be accepted once.')}>{invitationResult ? <div className="token-result"><div className="delete-record-summary"><span>{t('Recipient')}</span><strong>{invitationResult.invitation.email}</strong><small>{t('Workspace role')}: {t(roles.data?.find((role) => role.id === invitationResult.invitation.roleId)?.name ?? (canReadRoles ? 'Role unavailable' : 'Member'))}</small></div><p>{t(invitationResult.emailDelivered ? 'The invitation was emailed successfully. You can also copy the link below.' : 'Email delivery is not configured. Copy this link and share it with the invited person through a secure channel.')}</p><code>{invitationResult.invitationUrl}</code><Button variant="secondary" onClick={copyInvitationLink}><Copy size={14} /> {t(invitationCopied ? 'Link copied' : 'Copy invitation link')}</Button><div className="dialog-actions"><Button variant="primary" onClick={() => setInviteOpen(false)}>{t('Done')}</Button></div></div> : <form className="dialog-form" onSubmit={submitInvite}><FloatingInput label={t('Email address')} name="email" type="email" autoComplete="email" required autoFocus />{canReadRoles ? <OrganizationRolePicker roles={activeRoles.data ?? []} loading={activeRoles.isPending} error={activeRoles.isError} retrying={activeRoles.isFetching} onRetry={() => void activeRoles.refetch()} /> : <p>{t('This invitation uses the default Member role. The server checks whether you can assign it.')}</p>}<small>{t('The person will use the invitation link to create an account or sign in.')}</small>{invite.error && <div className="form-error" role="alert">{invite.error.message}</div>}<div className="dialog-actions"><Button type="button" variant="ghost" onClick={() => setInviteOpen(false)}>{t('Cancel')}</Button><Button type="submit" variant="primary" disabled={invite.isPending || (canReadRoles && (activeRoles.isPending || activeRoles.isError || !(activeRoles.data ?? []).some((role) => role.canAssign)))}>{t(invite.isPending ? 'Creating…' : 'Create invitation')}</Button></div></form>}</Dialog>
    ...
    <div className="profile-stack">
      ...
      <Surface className="profile-card">
        ...
        <form className="profile-form two-columns" key={profile.data?.displayName} onSubmit={saveProfile}>
          <FloatingInput label={t('Display name')} name="displayName" defaultValue={profile.data?.displayName} maxLength={120} required />
          <FloatingInput label={t('Email address')} type="email" value={profile.data?.email ?? ''} readOnly description={t(profile.data?.emailConfirmed ? 'Verified' : 'Unverified')} />
...
export function PlatformOrganizationsPage() {
  ...
  function submitCreate(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); create.mutate({ name: String(data.get('name')), slug: String(data.get('slug')), administratorEmail: String(data.get('administratorEmail')), placement: 'shared' }) }
