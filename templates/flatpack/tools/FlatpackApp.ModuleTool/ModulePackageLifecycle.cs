...
public sealed partial class ModuleWorkspace
{
    public ModuleDoctorReport RegisterWorkspace(string manifestPath)
    {
        ...
        {
            return new([], ["A workspace module manifest must be inside the Flatpack workspace."]);
        }
        ...
        if (catalog.Modules.Any(module => string.Equals(module.Id, manifest.Id, StringComparison.Ordinal)))
            return new([], [$"Flatpack module '{manifest.Id}' is already registered."]);
    ...
    public ModuleDoctorReport InstallPackage(string manifestPath, string expectedSha256)
    {
        ...
        if (catalog.Modules.Any(module => string.Equals(module.Id, candidate.Manifest.Id, StringComparison.Ordinal)))
            return new([], [$"Flatpack module '{candidate.Manifest.Id}' is already installed. Use 'module upgrade' for a package update."]);
    ...
    public ModuleDoctorReport UpgradePackage(string manifestPath, string expectedSha256)
    {
        ...
        if (current is null)
            return ToReport(currentModules, [$"Flatpack module '{candidate.Manifest.Id}' is not installed."]);
    ...
    public ModuleDoctorReport Unregister(string moduleId)
    {
        ...
        if (target is null)
            return ToReport(modules, [$"Unknown Flatpack module '{moduleId}'."]);
        ...
        if (catalog.Modules.Count == 1)
            return ToReport(modules, ["The final Flatpack module cannot be unregistered from the catalog."]);
    ...
    private string PackageManifestPath(ModuleManifest manifest) => ResolveInsideRoot(
        Path.Combine(".flatpack", "modules", manifest.Id, manifest.Version, "flatpack.module.json"));
        ...
        List<string> errors)
    {
        ...
        string apiPath = ResolveInsideRoot("src/FlatpackApp.Api/FlatpackApp.Api.csproj");
        string migratorPath = ResolveInsideRoot("src/FlatpackApp.Migrator/FlatpackApp.Migrator.csproj");
    ...
    private void UpsertPackageReferences(ModuleManifest manifest)
    {
        ...
        UpsertPackageReference(ResolveInsideRoot("src/FlatpackApp.Api/FlatpackApp.Api.csproj"), dotnet.Id);
        UpsertPackageReference(ResolveInsideRoot("src/FlatpackApp.Migrator/FlatpackApp.Migrator.csproj"), dotnet.Id);
    ...
    private void RemovePackageReferences(ModuleManifest manifest, ModuleCatalogFile remainingCatalog)
    {
        ...
        if (!dotnetStillUsed)
        {
            ...
            RemovePackageReference(ResolveInsideRoot("src/FlatpackApp.Api/FlatpackApp.Api.csproj"), dotnet.Id);
            RemovePackageReference(ResolveInsideRoot("src/FlatpackApp.Migrator/FlatpackApp.Migrator.csproj"), dotnet.Id);
        }
    ...
    private void RestorePackageGraphs(bool includeWeb)
    {
        WorkspaceCommandResult dotnet = _commandRunner.Run(
            "dotnet",
            ["restore", "FlatpackApp.slnx", "--force-evaluate"],
            _root);
        ...
        IEnumerable<string> packageLocks)
    {
        ...
        [
            ...
            ResolveInsideRoot("src/FlatpackApp.Api/FlatpackApp.Api.csproj"),
            ResolveInsideRoot("src/FlatpackApp.Migrator/FlatpackApp.Migrator.csproj")
        ];
        if (HasWebSurface())
        {
