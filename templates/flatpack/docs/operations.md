...
Telemetry flows once: Serilog writes JSON to stdout and exports logs through OTLP; the OpenTelemetry SDK exports traces and metrics. The Collector batches and retries, sends logs to Loki, traces to Tempo, and exposes metrics for Prometheus. Grafana data sources and the starter overview dashboard are provisioned from source. Jaeger is a supported alternative to Tempo, not an additional default service.
