...
{
    public async Task InvokeAsync(HttpContext context, PlatformDbContext platformDbContext)
    {
        ...
        await platformDbContext.Database.ExecuteSqlInterpolatedAsync(
            $"SELECT set_config('app.actor_id', {actor}, true), set_config('app.platform_admin', {platformAdministrator}, true)",
