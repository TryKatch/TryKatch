...
```bash
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.10
...
```bash
dotnet new install Trykatch.Templates@0.1.0-preview.10
```
...
dotnet new install artifacts/packages/Trykatch.Templates.0.1.0-preview.10.nupkg --force
dotnet new trykatch -n Horizon
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.10 --add-source artifacts/packages
```
