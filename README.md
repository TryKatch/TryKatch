...
```bash
dotnet tool install --global Trykatch.Cli
...
dotnet new install artifacts/packages/Trykatch.Templates.0.1.0-preview.1.nupkg --force
dotnet new trykatch -n Horizon
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.1 --add-source artifacts/packages
```
