...
```bash
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.19
...
```bash
dotnet new install Trykatch.Templates@0.1.0-preview.19
```
...
dotnet new install artifacts/packages/Trykatch.Templates.0.1.0-preview.19.nupkg --force
dotnet new trykatch -n Horizon --allow-scripts yes
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.19 --add-source artifacts/packages
```
