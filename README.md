...
```bash
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.22
...
```bash
dotnet new install Trykatch.Templates@0.1.0-preview.22
```
...
fails. See the [module authoring guide](https://docs.trykatch.net/modules/authoring/)
...
dotnet new install artifacts/packages/Trykatch.Templates.0.1.0-preview.22.nupkg --force
dotnet new trykatch -n Horizon --allow-scripts yes
dotnet tool install --global Trykatch.Cli --version 0.1.0-preview.22 --add-source artifacts/packages
```
