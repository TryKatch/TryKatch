...
If that command fails, restart Docker Desktop and wait until its engine is running. Resume the engine if Docker Resource Saver paused it. Then stop and restart the AppHost. PostgreSQL cannot start while the container runtime is unhealthy, so the migrator and API correctly remain in **Waiting**.
