...
Each organization opts in from **Administration → Settings → AI Configuration**. AI Help defaults to off. An Owner or delegated role with `organizations.manage` can select its subscription provider, enter the exact model ID, approved endpoint, write-only API key and timeout, enable AI and save. Then use **Test connection**, which sends a tiny billable request without business data or tools. An empty replacement field keeps a saved key; changing provider/destination requires a replacement for enabled key-dependent providers. Disable AI before explicitly removing a required key. Readers with `organizations.read` see metadata, never saved keys. Apply forward platform migrations via normal Migrator startup. Production uses organization-bound encryption and the certificate-encrypted persistent Data Protection ring: preserve its certificates/passwords and backups. Operators control `Assistant:AllowTenantConfiguration` and enroll trusted HTTPS bases in `Assistant:AllowedTenantEndpoints`. Native support covers OpenAI Responses, compatible Chat Completions (including DeepSeek) and Ollama, not every vendor protocol. Without an organization override, an explicitly enabled platform provider can still be used after opt-in. Invalid overrides never fall back. Public/anonymous AI and daily billing quotas remain excluded. Concurrent edits require an explicit reload.
