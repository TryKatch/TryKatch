...
Les certificats actifs doivent utiliser des clés privées distinctes ; le démarrage refuse les certificats copiés ou réémis partageant une clé publique. Data Protection refuse aussi les doublons parmi les clés conservées et la répétition de sa clé active dans `DecryptionCertificates`. Le certificat actif déchiffre déjà : lorsque B devient actif pendant une rotation, remplacez son entrée de déchiffrement par A.
...
L’exécution et la maintenance partagent une projection PostgreSQL ordonnée qui contrôle chaque ligne avec `octet_length` et limite le XML à 1 Mio par enregistrement, y compris le texte multioctet. Une ligne trop grande, vide ou nulle fait retourner uniquement les identifiants et des marqueurs d’échec nuls pour tout le résultat : aucun XML n’est matérialisé côté client. Les lignes refusées restent intactes. Cette limite par enregistrement n’est pas un quota global de clés.
...
Les invitations et les réinitialisations du mot de passe partagent un modèle HTML aux couleurs de l’application : styles intégrés, bouton d’action, URL de secours et alternative texte. L’invitation indique le nom lisible du rôle choisi. Les valeurs par défaut suivent le nom de l’application générée et le style bleu/neutre du frontend. Configurez `Email:Branding:ApplicationName`, `AccentColor` (hexadécimal à six chiffres) et, facultativement, `LogoUrl` (HTTPS sans identifiants intégrés) ; les variables d’environnement utilisent deux traits de soulignement. Alignez-les avec la marque du frontend, pas avec le thème personnel du destinataire. Les données sont encodées en HTML et une configuration invalide bloque le démarrage.

Utilisez Mailpit en local et un service SMTP compatible après déploiement. Pour commencer avec une offre gratuite, [SMTP2GO](https://support.smtp2go.com/hc/en-gb/articles/223087627-SMTP-Settings) utilise `mail.smtp2go.com:587`, `Security=StartTls`, un expéditeur vérifié et les identifiants d’un utilisateur SMTP conservés côté serveur. Vérifiez ses [limites mensuelles et quotidiennes actuelles](https://support.smtp2go.com/hc/en-gb/articles/223087947-Free-Plan). Désactivez le suivi des liens d’invitation/réinitialisation. Validez séparément l’authentification du domaine, les quotas, les rebonds et la réception réelle ; l’adaptateur ne provisionne pas automatiquement de compte fournisseur.
