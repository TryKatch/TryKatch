...
Chaque organisation choisit depuis **Administration → Settings → AI Configuration**. AI Help est désactivé par défaut. Le propriétaire ou un rôle délégué avec `organizations.manage` choisit son fournisseur, saisit l’identifiant exact du modèle, une destination approuvée, la clé en écriture seule et le délai, puis active et enregistre. **Test connection** envoie une petite requête potentiellement facturée, sans données métier ni outils. Une saisie de remplacement vide conserve la clé ; changer de fournisseur/destination exige une nouvelle clé pour un fournisseur actif qui en nécessite une. Désactivez l’IA avant de supprimer explicitement cette clé. `organizations.read` permet de consulter les métadonnées, jamais la clé enregistrée. Appliquez les migrations via le Migrator normal. La production utilise un chiffrement lié à l’organisation et le trousseau Data Protection persistant protégé par certificat : conservez certificats, mots de passe et sauvegardes. L’opérateur contrôle `Assistant:AllowTenantConfiguration` et les bases HTTPS fiables de `Assistant:AllowedTenantEndpoints`. OpenAI Responses, Chat Completions compatibles (dont DeepSeek) et Ollama sont pris en charge, pas tous les protocoles fournisseurs. Sans configuration propre, le fournisseur plateforme explicitement activé reste utilisable après consentement de l’organisation ; une configuration propre invalide n’utilise jamais ce secours. IA publique/anonyme et quotas journaliers restent exclus. Un conflit exige un rechargement explicite.

L’aide IA accompagne l’utilisateur dans l’application terminée : expliquer une fonctionnalité simplement, proposer les prochaines étapes documentées et poursuivre l’échange en français ou en anglais. La suggestion **Commencer** présente l’espace et ses fonctionnalités activées. Les instructions respectent l’accès de l’utilisateur ; l’assistant n’effectue aucune modification à sa place. Pour les processus propres au produit, comme les coopératives ou la faisabilité minière de Kamenta, fournissez des guides relus plutôt que d’attendre des exemples du socle qu’ils décrivent un processus métier non implémenté.

Une réponse du modèle peut demander plusieurs lectures. Le runtime valide tout le lot avant la première lecture, puis exécute les appels autorisés séquentiellement en vérifiant à nouveau les permissions. Un tour utilisateur autorise au maximum quatre lectures au total ; les écritures et les relances automatiques restent exclues.
